...
import { formatYear, truncateText } from "@/araclar/helpers";
...
import type { TMDBTVShowDetails } from "@/tipler/tmdb";

// Dizi Detay Sayfası İstemci Bileşeni
export function DiziDetayIstemcisi({ show }: { show: TMDBTVShowDetails }) {
  const [showFullOverview, setShowFullOverview] = useState(false);
  const { setTrailerKey, setIsTrailerOpen } = useAppStore();


  const cast = show.credits?.cast?.slice(0, 12) || [];
  const watchProvidersTR = show.watch_providers?.results?.["TR"];
    ...
    <div className="min-h-screen">
      ...
      <div className="container-custom -mt-40 relative z-10">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          >
            <div className="w-48 md:w-56 mx-auto md:mx-0 rounded-2xl overflow-hidden shadow-2xl border border-white/10">
              <Image
          ...
          >
            ...
            {/* Başlık */}
            <h1 className="text-3xl md:text-5xl font-black leading-tight mb-2">{show.name}</h1>
            {show.tagline && (
              <p className="text-gray-400 italic text-sm mb-4">"{show.tagline}"</p>
            )}

            {/* Meta Bilgiler */}
            <div className="flex flex-wrap items-center gap-4 mb-4">
              <PuanRozeti rating={show.vote_average} showStars />
            ...
            <div className="mb-4">
              <span className={`px-3 py-1 text-xs font-bold rounded-full ${
                ...
                {show.in_production ? "🟢 Devam Ediyor" : "⚫ Tamamlandı"}
              </span>
            </div>
            ...
            <div className="mb-8 max-w-4xl">
              <p className="text-gray-200 text-lg md:text-xl leading-relaxed">
                {showFullOverview ? show.overview : truncateText(show.overview, 250)}
              </p>
            ...
            {show.networks?.length > 0 && (
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-600">Yayınlandığı Yer:</span>
                {show.networks.slice(0, 3).map((network) => (
                  <span key={network.id} className="text-xs text-gray-400 font-medium">
                    {network.name}
                  </span>
                ))}
              </div>
        ...
        {show.seasons?.length > 0 && (
          <motion.div
            ...
            className="mt-10"
          >
            <h3 className="section-title text-xl mb-5">Sezonlar</h3>
            <div className="flex gap-4 overflow-x-auto pb-4 scroll-row">
              {show.seasons.filter((s) => s.season_number > 0).map((season) => (
                <div key={season.id} className="flex-shrink-0 w-36 group">
                  <div className="aspect-[2/3] rounded-xl overflow-hidden border border-white/8 group-hover:border-[#e50914]/30 transition-all mb-2 bg-[#1a1a1a]">
                    {season.poster_path ? (
                      <Image
                  ...
                  <p className="text-xs font-semibold text-white truncate">{season.name}</p>
                  <p className="text-xs text-gray-500">{season.episode_count} bölüm</p>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        {cast.length > 0 && (
          <motion.div
            ...
            className="mt-10"
          >
            <h3 className="section-title text-xl mb-5">Oyuncular</h3>
            <div className="flex gap-4 overflow-x-auto pb-4 scroll-row">
              {cast.map((actor) => (
